...
9. 사진 위치에는 [사진]을 표시한다.
10. 관련 글이 있으면 자연스럽게 내부링크 위치를 제안한다.
11. CPA가 없으면 CPA 문구를 억지로 넣지 않는다.
12. CPA가 있으면 정보 중간에 자연스럽게 CTA 위치를 표시한다.
13. 실제 링크가 있는 경우 [CPA_LINK_1], [CPA_LINK_2]처럼 표시한다.
14. 실제 링크가 없는 경우 가짜 URL을 만들지 않는다.
15. 제목은 검색자가 실제로 입력할 만한 표현을 우선한다.
16. 제목 후보 5개를 만들고 가장 좋은 1개를 추천한다.
...
# 본문
여기부터 네이버 블로그에 바로 복사할 수 있는 완성 원고를 작성한다.
